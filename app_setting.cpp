...
static void mpu_calibrate_btn_cb(lv_event_t* e) {
  mpuCalibrate(); // blocking ~1 detik, HP harus diam & rata -- toast-nya sendiri
}
...
void settingScreenShow() {
  ...
  // --- Kalibrasi Sensor Gerak (MPU6050) ---
  lv_obj_t* mpuBtn = lv_obj_create(content);
  lv_obj_remove_style_all(mpuBtn);
  lv_obj_set_size(mpuBtn, 300, 30);
  lv_obj_set_style_radius(mpuBtn, 6, 0);
  lv_obj_set_style_bg_color(mpuBtn, c565(g_mpuReady ? T().surface2 : T().danger), 0);
  lv_obj_set_style_bg_opa(mpuBtn, LV_OPA_COVER, 0);
  if (g_mpuReady) {
    lv_obj_add_flag(mpuBtn, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_add_event_cb(mpuBtn, mpu_calibrate_btn_cb, LV_EVENT_CLICKED, NULL);
  }
  lv_obj_t* mpuLbl = lv_label_create(mpuBtn);
  lv_label_set_text(mpuLbl, g_mpuReady ? "Kalibrasi Sensor Gerak (MPU6050)" : "MPU6050 tidak terdeteksi");
  lv_obj_set_style_text_color(mpuLbl, c565(g_mpuReady ? T().text : lv_color_white()), 0);
  lv_obj_set_style_text_font(mpuLbl, &lv_font_montserrat_14, 0);
  lv_obj_center(mpuLbl);
  ...
  lv_label_set_text(note,
    "Pilihan Font belum tersedia di versi LVGL ini -- custom font loader "
