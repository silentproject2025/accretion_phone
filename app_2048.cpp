...
static void buildUI() {
  ...
  // cell/gap diperkecil dari versi sblmnya (44/5 -> 35/4) -- ukuran lama
  // bikin papan 201px tinggi diposisikan mulai y=46, jadi bagian bawahnya
  // di y=247 = KELEWAT tinggi layar fisik (240px)! Ukuran baru: 4*35+5*4=160,
  // dari y=46 -> berakhir di y=206, pas di dalam batas aman.
  const int cell = 35, gap = 4;
  ...
  for (int r = 0; r < 4; r++) {
    for (int c = 0; c < 4; c++) {
      ...
      if (v > 0) {
        ...
        lv_obj_set_style_text_font(vl, &lv_font_montserrat_14, 0); // diperkecil biar muat di cell 35px
        lv_obj_center(vl);
      }
    }
  }
