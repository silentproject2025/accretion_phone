...
void chromeShowToast(const char* text) {
  ...
  // Sengaja pakai lv_layer_top() (layer global LVGL yg SELALU ada &
  // TIDAK IKUT KEHAPUS pas ganti layar), bukan lv_scr_act(). Kalau toast
  // jadi child dari layar aktif dan user pindah layar (navBack/tap app
  // lain) SEBELUM animasi toast ini kelar (~1.6 detik), layar lama bakal
  // dihapus sementara animasi toast masih jalan -- lebih aman taruh di
  // layer yg gak pernah dihapus sama sekali drpd gantung ke layar yg bisa
  // hilang kapan aja.
...
lv_obj_t* chromeCreateAppScreen(const char* title) {
  ...
  // Fade halus antar layar (200ms) -- auto_del=true berarti layar LAMA
  // otomatis kehapus SENDIRI oleh LVGL stlh animasi ini kelar. Krn ini,
  // caller (nav.cpp, atau rebuildSelf() di app manapun) TIDAK BOLEH lagi
  // hapus manual layar lama-nya sendiri -- kalau dobel, bisa nghapus objek
  // yg udah dihapus (use-after-free/crash).
